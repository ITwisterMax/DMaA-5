window.onload = function () {
  ...
  while (ok && iterationsCount++ < 1000) {
    ok = false;
    for (var i = 0; i < 4; i++) {
      result = result.sumArray(potentialFunction(points[i]).map((el) => el * correction));
  ...
  document.getElementById('res').innerHTML = '<font color="red">Separating function: </font>' + resultToString(result);
  ...
  btn0.onclick = function () {
    ...
    document.getElementById('text').innerHTML = '<font color="red">Belongs to class ' + (temp + 1) + '</font>';
  ...
  function potentialFunction(point) {
    var temp = Array(4);
    temp[0] = 4 * point.x;
    temp[1] = 4 * point.y;
